...
function getSystem()
{
	...
	$result = $dbh->query("select * from system");
	$row = $result->fetch();
...
function updateSystem($id,$websitename,$slogan,$currencyen,$currencyar,$paypalcurrency,$paiementemail,$logo)
{
	include("../libs/config.php");
	include("../libs/opendb.php");
	$stmt = $dbh->prepare("update system set websitename = '$websitename',slogan = '$slogan',currencyen = '$currencyen',currencyar = '$currencyar',paypalcurrency = '$paypalcurrency',paiementemail = '$paiementemail',logo = '$logo' where id = '$id'");
	$stmt->execute();	
	include("../libs/closedb.php");
}
