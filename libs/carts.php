...
function countUserSale($uid)
{
	include("libs/config.php");
	include("libs/opendb.php");
	$result = $dbh->query("select count(*) as count from sales where uid = '$uid' and delivered = 0");
	$row = $result->fetch();
	include("libs/closedb.php");
	if(empty($row) || $row['count'] < 3) return 1;
	else return 0;
...
function getCarts($lang,$ipaddress,$startResults,$resultsPerPage)
{
	include("libs/config.php");
	include("libs/opendb.php");
	$result = $dbh->query("select id,product,title{$lang} as title,price,quantity from cart where ipaddress = '$ipaddress' LIMIT $startResults, $resultsPerPage");
	$allrows = array();	
	if(!empty($result))
	{
		for($i=0; $row = $result->fetch(); $i++)
...
function getUserCarts($lang,$uid,$startResults,$resultsPerPage)
{
	include("libs/config.php");
	include("libs/opendb.php");
	$result = $dbh->query("select id,product,title{$lang} as title,price,quantity from cart where uid = '$uid' LIMIT $startResults, $resultsPerPage");
	$allrows = array();	
	if(!empty($result))
	{
		for($i=0; $row = $result->fetch(); $i++)
